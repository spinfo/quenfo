...
public class Edge<T, S extends Iterable<T>> implements Iterable<T> {
	...
	public Sequence<T, S> getSequence() {
		return sequence;
	}
	...
	public String getStringFromRoot(){
		String s = this.toString();
		Node<T, S> parent = this.getParentNode();
		while (parent.getIncomingEdge() != null) {
			s = parent.getIncomingEdge().toString() + s;
			parent = parent.getIncomingEdge().getParentNode();
		}
